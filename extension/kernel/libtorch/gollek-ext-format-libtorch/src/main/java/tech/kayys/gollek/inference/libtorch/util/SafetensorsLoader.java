 ...
 * 
 * <pre>{@code
 * Map<String, Tensor> weights = loader.loadAll(path);
 * Tensor embedding = loader.loadTensor(path, "model.embed_tokens.weight");
 * }</pre>
 */
@ApplicationScoped
public class SafetensorsLoader {
     ...
     */
    public Map<String, Tensor> loadAll(Path path) throws IOException {
        ...
        try (FileChannel channel = FileChannel.open(path, StandardOpenOption.READ)) {
            for (var entry : metadata.entrySet()) {
                try {
                    Tensor tensor = mapTensorFromChannel(channel, entry.getValue());
     ...
     */
    public Tensor loadTensor(Path path, String tensorName) throws IOException {
        Map<String, SafetensorsHeaderParser.TensorMetadata> metadata = headerParser.parse(path);

        SafetensorsHeaderParser.TensorMetadata info = metadata.get(tensorName);
        if (info == null) {
            throw new IllegalArgumentException(
                    "Tensor '" + tensorName + "' not found in " + path.getFileName()
    ...
    private Tensor mapTensorFromChannel(FileChannel channel,
