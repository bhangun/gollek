...
public class LibTorchAdapterApplier {
    ...
    public int applyRuntimeLora(TorchScriptRunner runner, Path adapterPath, float scale) {
        ...
        for (var entry : pairs.entrySet()) {
            ...
            try (Tensor loraA = safetensorsLoader.loadTensor(normalizedPath, pair.aKey());
                    Tensor loraB = safetensorsLoader.loadTensor(normalizedPath, pair.bKey());
    ...
    static Map<String, LoraPairKeys> discoverPairNames(Set<String> tensorNames) {
        ...
        for (String key : tensorNames) {
            ...
            String bKey = tensorNames.contains(bWithWeight) ? bWithWeight : (tensorNames.contains(bPlain) ? bPlain : null);
            if (bKey == null) {
                continue;
            }
