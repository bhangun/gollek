...
/**
 * Layer Normalization.
 * Mirrors {@code torch::nn::LayerNorm}.
 */
public class LayerNorm extends Module {
    ...
    public LayerNorm(long[] normalizedShape, double eps, boolean elementwiseAffine) {
        ...
        if (elementwiseAffine) {
            registerParameter("weight", Tensor.ones(normalizedShape, ScalarType.FLOAT, Device.CPU));
            registerParameter("bias", Tensor.zeros(normalizedShape, ScalarType.FLOAT, Device.CPU));
        }
    }
