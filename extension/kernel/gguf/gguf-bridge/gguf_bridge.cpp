...
static const char *QUANTIZATION_TYPES[] = {
    ...
    "q8_0",   "q8_1",   "q2_k",   "q3_k_s", "q3_k_m", "q3_k_l",
    "q4_k_s", "q4_k_m", "q5_k_s", "q5_k_m", "q6_k",   nullptr};
...
static bool extract_model_info(const fs::path &path, gguf_model_info_t *info) {
  ...
  // Simple JSON parsing for key fields (production code should use proper JSON
  // library)
  std::string line;
  ...
  if (config.count("model_type")) {
    strncpy(info->model_type, config["model_type"].c_str(),
            sizeof(info->model_type) - 1);
  }
  if (config.count("architectures")) {
    strncpy(info->architecture, config["architectures"].c_str(),
...
int gguf_validate_input(gguf_ctx_t ctx, gguf_model_info_t *info) {
  ...
  if (format.empty()) {
    set_error("Could not detect model format from: " +
              std::string(ctx->params.input_path));
...
int gguf_convert(gguf_ctx_t ctx) {
  ...
  try {
    ...
    if (!valid_quant) {
      set_error(std::string("Invalid quantization type: ") +
                ctx->params.quantization);
    ...
    // Determine if we're converting from HF format to GGUF or quantizing an
    // existing GGUF
    std::string input_ext = fs::path(input_path).extension().string();

    if (fs::is_directory(input_path) || input_ext == ".bin" ||
        input_ext == ".safetensors") {
      ...
      if (result != 0) {
        set_error("Python conversion script failed with code: " +
      ...
      // Quantizing an existing GGUF file using llama.cpp's quantization
      // function
      ...
      if (quantization == "f32")
        target_ftype = LLAMA_FTYPE_ALL_F32;
      else if (quantization == "f16")
        target_ftype = LLAMA_FTYPE_MOSTLY_F16;
      else if (quantization == "q8_0")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q8_0;
      else if (quantization == "q4_0")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q4_0;
      else if (quantization == "q4_1")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q4_1;
      else if (quantization == "q5_0")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q5_0;
      else if (quantization == "q5_1")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q5_1;
      else if (quantization == "q2_k")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q2_K;
      else if (quantization == "q3_k_s")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q3_K_S;
      else if (quantization == "q3_k_m")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q3_K_M;
      else if (quantization == "q3_k_l")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q3_K_L;
      else if (quantization == "q4_k_s")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q4_K_S;
      else if (quantization == "q4_k_m")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q4_K_M;
      else if (quantization == "q5_k_s")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q5_K_S;
      else if (quantization == "q5_k_m")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q5_K_M;
      else if (quantization == "q6_k")
        target_ftype = LLAMA_FTYPE_MOSTLY_Q6_K;
      ...
      uint32_t result = llama_model_quantize(input_path.c_str(),
                                             output_path.c_str(), &params);

      if (result != 0) {
        set_error("llama.cpp quantization failed with code: " +
                  std::to_string(result));
        llama_backend_free();
        return GGUF_ERROR_CONVERSION_FAILED;
      }
...
void gguf_free_context(gguf_ctx_t ctx) {
  ...
  if (ctx->params.input_path)
    free(const_cast<char *>(ctx->params.input_path));
  if (ctx->params.output_path)
    free(const_cast<char *>(ctx->params.output_path));
  if (ctx->params.model_type)
    free(const_cast<char *>(ctx->params.model_type));
  if (ctx->params.quantization)
    free(const_cast<char *>(ctx->params.quantization));
  if (ctx->params.vocab_type)
    free(const_cast<char *>(ctx->params.vocab_type));
