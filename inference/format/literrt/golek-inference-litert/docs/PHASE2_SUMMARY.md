...
Uni<AsyncJobStatus> getJobStatus(String jobId, String tenantId)
Multi<StreamingInferenceChunk> inferStream(InferenceRequest request)
Uni<List<InferenceResponse>> batchInfer(BatchInferenceRequest, String tenantId)
```
...
```java
boolean checkAndIncrementQuota(UUID tenantId, String resource, long amount)
boolean checkRateLimit(String tenantId, int requestsPerSecond)
boolean checkConcurrentRequests(String tenantId, int maxConcurrent)
boolean tryAcquireToken(String tenantId, int tokensPerSecond)
```

**Redis Integration**:
```java
// Sliding window with INCR + TTL
stringCommands.incr("ratelimit:" + tenantId + ":rps")
...
**Usage**:
```java
metrics.recordSuccess(tenantId, modelId, runnerName, latencyMs);
metrics.recordFailure(tenantId, modelId, errorType);
metrics.recordInputSize(tenantId, modelId, bytes);
metrics.recordQuotaUsage(tenantId, resourceType, used, limit);
```

---

### 5. TenantContext & Filter ✅ COMPLETE

**Files**: 
- `TenantContext.java` (~70 lines)
- `TenantContextFilter.java` (~90 lines)
...
// Output: ... tenantId=acme requestId=req-123 Processing request
```
...
| TenantContext + Filter | ~160 | ✅ Complete |
...
1. HTTP Request arrives
   ↓
2. TenantContextFilter validates tenant
   ↓
...
Uni<InferenceResponse> inferAsync(InferenceRequest request) {
    return Uni.createFrom().item(() -> validateTenant(request.getTenantId()))
        .chain(tenant -> {
...
checkAndIncrementQuota(tenantId, "requests", 1)

// 2. Rate limiting (requests per second)
checkRateLimit(tenantId, 100)

// 3. Concurrent requests
checkConcurrentRequests(tenantId, 10)

// 4. Token bucket (smooth traffic)
tryAcquireToken(tenantId, 100)
```
...
public Uni<Response> infer(...) {
    ...
    // Output: tenantId=acme requestId=req-123 Request processed
}
```
