...
import tech.kayys.gollek.engine.ratelimit.SlidingWindowRateLimiter;
...
public abstract class CloudProviderAdapter extends AbstractProvider {
    ...
    tech.kayys.gollek.provider.core.quota.ProviderQuotaService quotaService;
    ...
    protected Throwable handleFailure(Throwable ex) {
        if (ex instanceof jakarta.ws.rs.WebApplicationException) {
            ...
            if (webEx.getResponse().getStatus() == 429) {
                ...
                return new tech.kayys.gollek.spi.routing.QuotaExhaustedException(id(), "Provider quota exhausted (429)");
            }
        }
