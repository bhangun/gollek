...
public class ModelRunnerFactoryTest {
    ...
    public static class TestModelRunner implements ModelRunner {
        ...
        @Override
        public void initialize(ModelManifest manifest, tech.kayys.gollek.engine.model.RunnerConfiguration config) {
        }

        @Override
        public tech.kayys.gollek.spi.inference.InferenceResponse infer(
                tech.kayys.gollek.spi.inference.InferenceRequest request) {
            return null;
        }

        @Override
        public java.util.concurrent.CompletableFuture<tech.kayys.gollek.spi.inference.InferenceResponse> inferAsync(
                tech.kayys.gollek.spi.inference.InferenceRequest request) {
            return null;
        }
    ...
    private ModelManifest createManifest(String requestId, String modelId) {
        return new ModelManifest(
                modelId, "model", "1.0", requestId, "test-api-key", requestId,
