...
public class InferenceOrchestrator {
        ...
        public io.smallrye.mutiny.Uni<InferenceResponse> executeAsync(
                ...
                span.setAttribute("tenant.id", tenantContext.getTenantId().value());

                LOG.infof("Orchestrating inference for model %s (tenant: %s)",
                                modelId, tenantContext.getTenantId().value());

                return router.route(modelId, request, tenantContext)
        ...
        public Multi<StreamChunk> streamExecute(
                ...
                return router.routeStream(modelId, request, tenantContext);
        }
}