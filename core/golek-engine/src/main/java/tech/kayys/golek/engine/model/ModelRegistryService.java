...
@Slf4j
@ApplicationScoped
public class ModelRegistryService implements tech.kayys.golek.api.model.ModelRegistry {

    @Inject
    ModelRepository modelRepository;

    @Inject
    tech.kayys.golek.api.storage.ModelStorageService storageService;
                        ...
                        .chain(existingVersion -> {
                            ...
                            return storageService.uploadModel(
                                    ...
                                    request.modelData()).map(storageUri -> {
                                        String checksum = calculateChecksum(request.modelData());
                                        ModelVersion version = ModelVersion.builder()
                                                ...
                                                .checksum(checksum)
                                                .sizeBytes((long) request.modelData().length)
                                ...
                                .map(versions -> {
                                    if (versions.isEmpty()) {
                                        throw new ModelException(
                                                ...
                                                "No active versions found for model: " + modelId,
                                                modelId);
                                    }
                                    return manifestFromEntity(model, versions.get(0));
                                });
                    } else {
