...
import tech.kayys.wayang.tenant.AuthenticationException;
...
public class ModelRegistryService implements tech.kayys.golek.spi.model.ModelRegistry {
    ...
    @Inject
    GolekModelStorageService storageService;
    ...
    private ModelManifest buildManifest(
            ...
            String checksum,
            Long sizeBytes) {
        Map<tech.kayys.golek.spi.model.ModelFormat, tech.kayys.golek.spi.model.ArtifactLocation> artifacts = buildArtifacts(
                request.framework(),
                storageUri,
                checksum,
                sizeBytes);
    ...
    private Map<tech.kayys.golek.spi.model.ModelFormat, tech.kayys.golek.spi.model.ArtifactLocation> buildArtifacts(
            String format,
            String storageUri,
            String checksum,
            Long sizeBytes) {
        ...
        tech.kayys.golek.spi.model.ModelFormat resolved = resolveFormat(format, storageUri);
        if (resolved == null) {
            return Collections.emptyMap();
        }

        return Map.of(resolved,
                new tech.kayys.golek.spi.model.ArtifactLocation(
                        storageUri,
                        checksum,
                        sizeBytes,
                        null));
    ...
    private tech.kayys.golek.spi.model.ModelFormat resolveFormat(String format, String storageUri) {
        if (format != null && !format.isBlank()) {
            String normalized = format.trim().toLowerCase();
            if ("litert".equals(normalized) || "tflite".equals(normalized)) {
                return tech.kayys.golek.spi.model.ModelFormat.LITERT;
            }
            return tech.kayys.golek.spi.model.ModelFormat.findByRuntime(format)
                    .or(() -> tech.kayys.golek.spi.model.ModelFormat.findByExtension(format))
                    .orElse(null);
        }
        ...
        if (storageUri != null) {
            ...
            if (dot >= 0 && dot + 1 < path.length()) {
                ...
                return tech.kayys.golek.spi.model.ModelFormat.findByExtension(ext).orElse(null);
            }
        }
