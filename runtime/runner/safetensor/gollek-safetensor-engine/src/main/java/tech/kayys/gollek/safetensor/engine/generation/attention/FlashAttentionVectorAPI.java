...
public class FlashAttentionVectorAPI {
    ...
    private static void computeHeadQuery(int b, int hq, int hkv, int i, 
                                        ...
                                        float scale, long seqLenKV, int headDim, boolean causal,
        ...
        for (int j = 0; j < seqLenKV; j++) {
            ...
            float score = AttentionFp32VectorMath.dotProduct(qSeg, qOff, kSeg, kOff, headDim) * scale;
            ...
            AttentionFp32VectorMath.updateAccumulator(softmax.accumulator(), vSeg, vOff, softmax.previousWeight(),
                    softmax.currentWeight(), headDim);
        }

        // 4. Final Normalization
        long oOff = ((((long)b * out.size(1) + hq) * out.size(2) + i) * headDim) * ValueLayout.JAVA_FLOAT.byteSize();
        AttentionFp32VectorMath.writeNormalizedAccumulator(oSeg, oOff / Float.BYTES, softmax.accumulator(),
                softmax.inverseNormalizer(), headDim);
    }
}
