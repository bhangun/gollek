...
public final class KaggleRepository implements ModelRepository {
    ...
    public ModelArtifact fetch(ModelDescriptor descriptor) {
        ...
        try {
            ...
            return new ModelArtifact(targetFile, null,
                    Map.of("format", descriptor.format(), "size", String.valueOf(size)));
        } catch (Exception e) {
                ...
                if (artifacts.isEmpty() && isAutoDownloadEnabled()) {
                    try {
                        ...
                        if (modelId.startsWith("kaggle:")) {
                            ...
                            LOG.debugf("Kaggle lookup failed for %s (not a Kaggle model or unauthorized): %s", slug,
                                    e.getMessage());
                        }
                    }
                }
    ...
    private int priority(String filename) {
        ...
        if (lower.endsWith(".safetensors"))
            return 80;
        if (lower.equals("pytorch_model.bin"))
            return 70;
    ...
    private ModelFormat detectFormat(String filename) {
        ...
        if (isSafetensorFile(filename))
            return ModelFormat.SAFETENSORS;
        if (isPytorchFile(filename))
            return ModelFormat.PYTORCH;
        if (isLitertFile(filename))
