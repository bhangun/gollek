...
import tech.kayys.alkhawarizm.spi.model.ModelArtifact;
import tech.kayys.alkhawarizm.spi.model.ModelDescriptor;
import tech.kayys.alkhawarizm.spi.model.ModelRef;
import tech.kayys.alkhawarizm.core.model.ModelFormat;
import tech.kayys.alkhawarizm.spi.model.ModelManifest;
import tech.kayys.alkhawarizm.spi.model.Pageable;
...
public final class HuggingFaceRepository implements ModelRepository {
            ...
            try {
                ...
                if (isAutoDownloadEnabled() && !"community".equals(requestId)) {
                    ...
                    Path downloaded = downloadBestArtifact(repoId, targetFolder,
                            tech.kayys.alkhawarizm.spi.model.PullOptions.DEFAULT);
                    if (downloaded != null) {
                        ...
                        manifest.setFiles(ManifestStore
                                .listBlobFiles(Files.isRegularFile(downloaded) ? downloaded.getParent() : downloaded));
    ...
    private ModelManifest toModelManifest(GollekManifest gm, String requestId) {
        ...
        tech.kayys.alkhawarizm.spi.model.ArtifactLocation artifact = new tech.kayys.alkhawarizm.spi.model.ArtifactLocation(
    ...
    @Override
    public Uni<ModelManifest> pull(String modelId, tech.kayys.alkhawarizm.spi.model.PullOptions options) {
        return Uni.createFrom().item(() -> {
            try {
                String repoId = normalizeRepoId(modelId);
                String revision = options.getRevision() != null ? options.getRevision()
                        : (config != null ? config.revision() : DEFAULT_REVISION);
                String manifestName = GollekManifest.computeName(repoId, revision);
    ...
    private Path downloadBestArtifact(String repoId, Path targetDir, tech.kayys.alkhawarizm.spi.model.PullOptions options)
            throws Exception {
        ...
        if (isPipeline) {
            ...
            // Smart Filter: Deduplicate formats within directories (prefer .safetensors
            // over .bin)
            ...
            for (String file : toDownload) {
                if (isCheckpointRelevant(file)) {
                    ...
                    if (options.isForce() || !Files.exists(target)) {
                        try {
                            client.downloadFile(repoId, file, options.getRevision(), target,
                                    progressPrinter(repoId, file));
                            downloadedCount++;
                        } catch (Exception e) {
            ...
            // If No obvious primary found but it's a pipeline, return the model_index as
            // anchor
    ...
    private boolean isCheckpointRelevant(String name) {
        ...
        return lower.endsWith(".safetensors") || lower.endsWith(".bin") || lower.endsWith(".pt")
                || lower.endsWith(".pth")
                || lower.endsWith(".json") || lower.endsWith(".model") || lower.endsWith(".txt")
                || lower.endsWith(".spm")
                || lower.endsWith(".msgpack") || lower.endsWith(".gguf") || lower.endsWith(".ckpt")
                || lower.endsWith(".onnx");
    }
