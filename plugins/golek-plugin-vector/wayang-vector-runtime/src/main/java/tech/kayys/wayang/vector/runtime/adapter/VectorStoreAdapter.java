package tech.kayys.wayang.vector.runtime.adapter;

import io.smallrye.mutiny.Uni;
import tech.kayys.gamelan.executor.memory.Memory;
import tech.kayys.gamelan.executor.memory.MemoryStatistics;
import tech.kayys.gamelan.executor.memory.ScoredMemory;
...
public class VectorStoreAdapter implements VectorMemoryStore {
    ...
    public Uni<String> store(Memory memory) {
        ...
        return vectorStore.store(List.of(vectorEntry))
            .onItem().transformToUni(v -> Uni.createFrom().item(memory.getId()));
    }
    ...
    public Uni<List<String>> storeBatch(List<Memory> memories) {
        ...
        return vectorStore.store(vectorEntries)
            .onItem().transformToUni(v -> Uni.createFrom().item(
    ...
    public Uni<Long> deleteNamespace(String namespace) {
        ...
        return vectorStore.deleteByFilters(filters)
            .onItem().transformToUni(v -> Uni.createFrom().item(1L));
    }
