 ...
 *
 * <p>Use {@link #withResourceMetrics()} for a provider that automatically
 * tracks CPU and memory per request via JVM MXBeans. Use {@link #noop()}
 * when observability isn't needed (e.g., in tests).
 */
public interface SdkObservabilityProvider {
    ...
    /**
     * Returns a provider backed by {@link DefaultSdkMetricsCollector} that
     * reads CPU and memory metrics directly from JVM MXBeans.
     * Suitable for embedded / local SDK usage without Micrometer.
     */
    static SdkObservabilityProvider withResourceMetrics() {
        DefaultSdkMetricsCollector collector = new DefaultSdkMetricsCollector();
    ...
    /**
     * A no-op provider for when observability isn't needed (e.g., in tests).
     */
    static SdkObservabilityProvider noop() {
        return new SdkObservabilityProvider() {
            private final SdkMetricsCollector noopCollector = new SdkMetricsCollector() {
