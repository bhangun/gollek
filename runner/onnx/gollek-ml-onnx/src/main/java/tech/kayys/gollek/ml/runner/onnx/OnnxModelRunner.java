 ...
 *
 * <p>
 * Integrates ONNX Runtime for high-performance inference via the C API
 * using Java 25 Panama Foreign Memory API. Supports CPU, CUDA, and TensorRT
 * execution providers.
 *
 * <h2>Native Library Requirements</h2>
 * <ul>
 * <li><b>CPU:</b> libonnxruntime.so / onnxruntime.dll /
 * libonnxruntime.dylib</li>
...
public class OnnxModelRunner implements ModelRunner {
    ...
    public InferenceResult infer(Map<String, InferenceInput> inputs) {
        ...
        try {
            ...
            for (MemorySegment v : inputValues)
                releaseOnnxValue(v);
            for (MemorySegment v : outputValues)
                releaseOnnxValue(v);
    ...
    private void loadModelFallback() {
        ...
        inputTensorInfo.put("input",
                new OnnxTensorInfo("input", new long[] { 1, 1000 }, InferenceInput.InputDataType.FLOAT32));
        outputTensorInfo.put("output",
                new OnnxTensorInfo("output", new long[] { 1, 1000 }, InferenceInput.InputDataType.FLOAT32));
    }
    ...
    private void runSessionNative(String[] inputNames, MemorySegment[] inputValues, int numInputs,
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            // For now, use sequential fallback since full FFM binding requires extensive
            // setup
            runSessionFallback(inputNames, inputValues, numInputs, outputNames, outputValues, numOutputs);
        }
    }
    ...
    private long getInputCount(MemorySegment session) {
        return 0;
    }

    private long getOutputCount(MemorySegment session) {
        return 0;
    }

    private String getInputName(MemorySegment session, int index, Arena arena) {
        return "input";
    }

    private String getOutputName(MemorySegment session, int index, Arena arena) {
        return "output";
    }
